...
let sadGames = huskyGames2016.filter(huskiesLost);
console.log(sadGames); 

//Log out an array of opponents that UW lost to. Hint: Use the `.map()` method 
//to extract the opponent names!
let sadOpponents = sadGames.map(extractOpponent);
console.log(sadOpponents); 
...
sadGames.forEach( function(game){
...
function mostYardsPassing(gameOne, gameTwo) {
  if (gameOne.passing_yards === undefined || gameTwo.passing_yards > gameOne.passing_yards) {
    return gameTwo;
  } else {
  return gameOne;
  }
...
let mostPassGame = huskyGames2016.reduce(mostYardsPassing, { } );
console.log(mostPassGame);
...
function makeCombinedFilter(funcOne, funcTwo, game) {
  function newFilter(game){
    let resOne = funcOne(game);
    let resTwo = funcTwo(game);
    if( resOne == true && resTwo == true){
      return resOne && resTwo;
    }
  }
  return newFilter(game);
}
...
function fumbledAndLostFilter(game){
  return makeCombinedFilter(huskiesLost, fumbleOccur, game);
}
...
let fumNLost = huskyGames2016.filter(fumbledAndLostFilter);
console.log(fumNLost);
